...
static uint8_t pn532_packetbuffer[13] = {0};

static bool sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout);
static void readdata(uint8_t *buffer, uint8_t length);
...
static uint32_t getFirmwareVersion(void) {
    ...
    pn532_packetbuffer[0] = PN532_COMMAND_GET_FIRMWARE_VERSION;
...
static void writecommand(uint8_t *cmd, uint8_t cmdlen) {
    ...
    packet[3] = LEN;
    packet[4] = ~LEN + 1;
    packet[5] = PN532_DIRECTION_HOST_TO_PN532;
    ...
    packet[6 + cmdlen] = ~(PN532_DIRECTION_HOST_TO_PN532 + sum) + 1;
